		...
		<body class="fond">
			...
			<div class="fond">
				...
				<?php
					...
					$req=$db->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM vehicule_achat LIMIT :limite OFFSET :debut"); 
					...
					while($donnees = $req->fetch())  // on affiche les voitures disponibles dans le catalogue
					{
						...
						if($donnees['nb_disponible']>0)   // si vehicule dispo
						{
							...
							{
								echo '<a href="carmania_achat.php?modele='.$donnees['modele'].'&id='.$donnees['idVehicule_achat'].'"><button class="w3-green w3-button">Acheter</button></a>';
						...
						if(isset($_SESSION['level']) && $_SESSION['level']==1) // si admin bouton pour supprimer le véhicule
						{
							...
							echo '<a href="carmania_catalogue_a.php?page='.$page.'"><button class="w3-green w3-button" onclick="sup('.$donnees['idVehicule_achat'].')">Supprimer</button></a>';
							//echo'<a href="carmania_sup.php?pk='.$donnees['idVehicule_achat'].'&mode=a">LEL</a>';
							//echo'</form>';
						}
						echo'</div>';
					...
					if($page >1)
						...
						echo'<a href="carmania_catalogue_a.php?page=1" class="w3-bar-item w3-button w3-text-green">&laquo;</a>';
			
					for($i=1; $i<=$nb_pages; $i++)
						echo'<a href="carmania_catalogue_a.php?page='.$i.'" class="w3-button w3-text-green">'.$i.'</a>';
			
					if($page < $nb_pages)
						echo'<a href="carmania_catalogue_a.php?page='.($page+1).'" class="w3-bar-item w3-button w3-text-green">&raquo;</a>';
					else
						echo'<a href="carmania_catalogue_a.php?page='.$nb_pages.'" class="w3-bar-item w3-button w3-text-green">&raquo;</a>';
