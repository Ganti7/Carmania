		...
		<body>
			<div id="centrer">
				<?php
					...
					if (empty($_POST['marque']))  // on est dans le formulaire
			
					{
						...
						<label for="prix" class="w3-text-green">Prix : </label><input class="input" type="text" name="prix"><br>
						</fieldset>
					...
					else
					{
						...
						if(!preg_match("#^[1-9][0-9]{1,5}$#", $prix))
						{
...

						{
							...
							$query=$db->prepare('INSERT INTO  vehicule_achat(prix_achat, carburant, puissance, marque,
							modele, transmission,chemin_image,climatisation,empreinte_carbone,nb_disponible,adresse_mail_utilisateur)
							VALUES (:prix, :carburant, :puissance, :marque, :modele, :transmission, :image, :climatisation, :carbone, :dispo, :mail)' );
							...
							// on prend l'id du dernier véhicule rentré dans le catalogue
							$req=$db->query('SELECT idVehicule_achat FROM vehicule_achat WHERE idVehicule_achat="'.$last_id.'"');
							$donnees = $req->fetch();
							$req->CloseCursor();
							
							// pour pouvoir rentrer ses informations dans la table 
							$query=$db->prepare('INSERT INTO voiture_achat (portes, couleur , idVehicule_achat )
							...
							header("refresh: 2;url = carmania_partic_vehic_a.php"); // redirige sur carmania.php après 2s			
						}
						
						else // si problème rencontré
						{
							echo'<p class="w3-text-green">Mise en vente interrompue</p>';
							...
							echo'<p class="w3-text-green">Cliquez <a href="carmania_partic_vente.php">ici</a> pour recommencer</p>';
						}
