	...
	<html class="fond">
		...
		<body id="centrer">
			...
			<?php
				...
				echo'<a href="carmania_partic_vehic_a.php" class="w3-bar-item w3-button w3-dark-grey">Mes véhicules mis en vente</a>';
				...
				$req=$db->prepare('SELECT SQL_CALC_FOUND_ROWS * FROM vehicule_achat WHERE adresse_mail_utilisateur="'.$mail.'" 
				...
				while($donnees = $req->fetch())  // on affiche les véhicules achetés
				{
					...
					{
						$requete=$db->query('SELECT * FROM commande WHERE adresse_mail_vendeur="'.$mail.'" AND prix_achat="'.$donnees['prix_achat'].'"
						 AND marque="'.$donnees['marque'].'" AND modele="'.$donnees['modele'].'"'); 
						$attr=$requete->fetch();
						echo'<p class="w3-text-green"> Acheté le : '.$attr['date_achat'].'</p>';
				...
				if($page >1)
					echo'<a href="carmania_partic_vehic_a.php?page='.($page-1).'" class="w3-bar-item w3-button w3-text-green">&laquo;</a>';
				else
					echo'<a href="carmania_partic_vehic_a.php?page=1" class="w3-bar-item w3-button w3-text-green">&laquo;</a>';
			
				for($j=1; $j<=$nb_pages; $j++)
					echo'<a href="carmania_partic_vehic_a.php?page='.$j.'" class="w3-button w3-text-green">'.$j.'</a>';
			
				if($page < $nb_pages)
					echo'<a href="carmania_partic_vehic_a.php?page='.($page+1).'" class="w3-bar-item w3-button w3-text-green">&raquo;</a>';
				else
					echo'<a href="carmania_partic_vehic_a.php?page='.$nb_pages.'" class="w3-bar-item w3-button w3-text-green">&raquo;</a>';
		...
		<script type="text/javascript" src="http://code.jquery.com/jquery-1.7.2.min.js"></script>
			<script>
				function sup(idv){  // function qui appelle script php pour supprimer annonce
					...
					var url="carmania_sup.php?pk="+idv+"&mode=a";
